...
class ASTGeneration(ZCodeVisitor):
    ...
    def visitIndexoperator(self, ctx: ZCodeParser.IndexoperatorContext):
        if ctx.ID():
            return Callstmt(ctx.ID().getText(),self.visit(ctx.indexope()))
    ...
    def visitIfstmt(self, ctx: ZCodeParser.IfstmtContext):
        if ctx.getChildCount() == 7:
            return If(self.visit(ctx.expr()), self.visit(ctx.statement()), self.visit(ctx.eliflist()))
    ...
    def funccallstmt(self, ctx: ZCodeParser.FunccallstmtContext):
        return CallExpr(ctx.ID().getText(), self.visit(ctx.argumentlist()))
    ...
    def visitArrayparameter(self, ctx: ZCodeParser.ArrayparameterContext):
        return ArrayType(self.visit(ctx.sizelist()), self.visit(ctx.typ()))
